...
module.exports = {
	...
	async execute(interaction) {
		...
		const song = {
			url: 'https://www.youtube.com/watch?v=oCX-a1r1Gzk',
			title: 'golira',
		};
		if (!talkedRecently.has(interaction.member.id)) {
			if (!serverQueue) {
				...
				try {
					...
					return interaction.reply('Golira feliz :)');
				}
				catch (error) {
					console.error('Erro:', error);
					queue.delete(interaction.guild.id);
					return interaction.reply('Golira triste :(');
				}
			}
			else {
				...
				return interaction.reply('Golira esperar fila :)');
			}
		}
		else {
			await interaction.reply('Usar esse comando agora vai matar o golira anterior, espere um pouco');
		}
	},
};