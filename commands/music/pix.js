...
module.exports = {
	...
	async execute(interaction) {
		...
		const song = {
			url: 'https://www.youtube.com/watch?v=_7MVMKe6HCI',
			title: 'Kurt Cobain pedindo pix',
		};
		if (!talkedRecently.has(interaction.member.id)) {
			if (!serverQueue) {
				...
				try {
					...
					return interaction.reply('Ta chegando o pix');
				}
				catch (error) {
					console.error('Erro:', error);
					queue.delete(interaction.guild.id);
					return interaction.reply('O pix falhou');
				}
			}
			else {
				...
				return interaction.reply('Daqui a pouco chega o pix');
			}
		}
		else {
