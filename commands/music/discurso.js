...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('discurso')
		.setDescription('Dr. Enéa Carneiro tem um lindo discurso a fazer'),
	async execute(interaction) {
		...
		const song = {
			url: 'https://www.youtube.com/watch?v=RAojzW6A5l0',
			title: 'Lindo discurso do Dr Enéas Carneiro sobre garotas palhaço',
		};
		if (!talkedRecently.has(interaction.member.id)) {
			if (!serverQueue) {
				...
				try {
					...
					return interaction.reply('Dr. Enéas tem algo importante para dizer');
				}
				catch (error) {
				...
				return interaction.reply('MEU NOME É ENÉAS');
			}
		}
		else {
